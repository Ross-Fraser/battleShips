...
Instructions
 ...
 2. Position your ships on the board.
    ...
    ii.     You can choose to position your ships either h (horizontal)
            or v (vertical), but not diagonal.
 ...
 3. Let the battle begin!
    ...
    ii.     ships are represented with an 's' if you hit a ship, you will
            see an 'x' on the board, otherwise you will see a '-'.
 4. The game ends when all of 1 navies ships are sunk or they run out of
            ammo.
 5. To exit the game, type 'exit' at any input prompt.
    """
...
class BattleShipBoard:
    ...
    def __init__(self, boards):
        ...
        try:
            ...
            if boards not in [5, 8]:
                raise ValueError(
                    Fore.YELLOW + "The board must be 5 or "
                                  "8, please try again.")
        except ValueError as e:
            raise ValueError(Fore.YELLOW + f"{e}")
    ...
    def print_board(self):
        """
        This function prints the board with row and column labels
        to the terminal.
        """
        print(Fore.WHITE + "\n   " + " "
...
def fire_ammo(board, ammo_count):
    for shot in range(ammo_count, 0, -1):
        ...
        try:
            ...
            col_range = string.ascii_uppercase[:board.boards]
        ...
        # Decrement the shot only if the input is valid
        if valid_input:
...
while True:
    try:
        ...
        if not boards.isdigit():
            raise ValueError(Fore.YELLOW + "no text or symbols "
                                           "allowed, please try again.")
        game_board = BattleShipBoard(boards)
        break
    except ValueError as e:
        print(Fore.YELLOW + f"Invalid input: {e}\n")
...
def get_user_ship_coordinates(board_size, ship_size, row_range, col_range):
    ...
    print(Fore.BLUE + f"Enter the details for the position of "
                      f"ship size-{ship_size}\n")

    while True:
        try:
            orientation = input(Fore.BLUE + "Enter orientation [H]orizontal "
                                            "or [V]ertical): ").upper()
            ...
            """
            input_coordinates = input(f"Enter starting row and "
                                      f"column: ").upper()

            if (len(input_coordinates) != 2
                    or not input_coordinates[0].isdigit()
                    or not input_coordinates[1].isalpha()):
                raise ValueError(Fore.YELLOW + "Please enter valid "
                                               "row and column values.")

            start_row, start_col = input_coordinates[0], input_coordinates[1]

            """
            Handle both cases where the first character is a
            letter or a number
            """
            if start_row.isalpha():
                start_row = row_range.index(start_row) + 1
            elif start_row.isdigit():
                start_row = int(start_row)
            else:
                raise ValueError(Fore.YELLOW + "Not a row value. "
                                               "Please try again.")

            if start_col not in col_range:
                raise ValueError(Fore.YELLOW + "Not a column value. "
                                               "Please try again.")
            ...
            if board_size == 5:
                ...
                ship = [(row, col_range.index(start_col))
                        for row in range(int(start_row),
                        int(start_row) + ship_size)]
            else:
            ...
            if orientation == 'H':
                ...
                if end_col >= len(col_range):
                    raise ValueError(Fore.YELLOW + "Ship goes beyond the "
                                                   "board boundaries. "
                                                   "Please try again.")
                ship = [(start_row, col)
                        for col in range(col_range.index(start_col),
                ...
                if end_row > board_size:
                    raise ValueError(Fore.YELLOW + "Ship goes beyond "
                                                   "the board "
                                                   "boundaries. "
                                                   "Please try again.")
                ship = [(row, col_range.index(start_col))
...
def create_ships(board_size, ammo):
    ...
    for size in ship_sizes:
        ...
        game_board.ships = ships
        for ship in ships:
            row, col = ship
            game_board.board[row - 1][col] = Fore.GREEN + 's'
...
ships = create_ships(game_board.boards,
                     ammo=AMMO_5 if game_board.boards == 5 else AMMO_8)
game_board.ships = ships
game_board.place_ships_on_board(ships)
fire_ammo(game_board, AMMO_5 if game_board.boards == 5 else AMMO_8)
