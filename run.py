    ...
    for shot in range(ammo_count, 0, -1):
        ...
        try:
            ...
            if len(target) != 2 or not target[0].isdigit() or not target[1].isalpha():
                raise ValueError(Fore.YELLOW + "Please enter valid row and column values.")
            
            row, col_text = int(target[:-1]), target[-1]

            col_range = [chr(ord('A') + i) for i in range(board.boards)]
            
            if not (1 <= row <= board.boards) or col_text not in col_range:
            ...
            if row not in range(1, board.boards + 1) or col_text not in col_range:
                raise ValueError(Fore.YELLOW + f"Please try again. "
...
def get_user_ship_coordinates(board_size, ship_size, row_range, col_range):
    ...
    while True:
        try:
            ...
            if len(input_coordinates) != 2 or not input_coordinates[0].isdigit() or not input_coordinates[1].isalpha():
                raise ValueError(Fore.YELLOW + "Please enter valid row and column values.")
            ...
            if start_row.isalpha():
                ...
                raise ValueError(Fore.YELLOW + "Not a row value. Please try again.")

            if start_col not in col_range:
                raise ValueError(Fore.YELLOW + "Not a column value. Please try again.")
            ...
            if orientation == 'H':
                ...
                if end_col >= len(col_range):
                    raise ValueError(Fore.YELLOW + "Ship goes beyond the board boundaries. Please try again.")
...
ships = create_ships(x.boards, ammo=AMMO_5 if x.boards == 5 else AMMO_8)
x.ships = ships
